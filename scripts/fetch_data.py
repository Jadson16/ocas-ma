...
OUT_FILE = DATA_DIR / "babacau_pevs.csv"
...
def main():
    ...
    print(f"Municípios           : {df['municipio_id'].nunique()}")
    print(f"Período              : {df['ano'].min()} – {df['ano'].max()}")
    n_prod = df["quantidade_ton"].notna().sum()
    print(f"Registros com produção: {n_prod} ({n_prod/len(df):.1%})")
    print()
    print(df[df["quantidade_ton"].notna()].head(10).to_string(index=False))
