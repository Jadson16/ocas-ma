"""
Coleta dados PEVS (piaçava - fibra) do SIDRA/IBGE para municípios do Maranhão.
...
STATE_CODE = 21
CLASSIFICATION = 193
CATEGORY = 3425          # Piaçava (fibra)
START_YEAR = 1995
END_YEAR = 2024

DATA_DIR = Path(__file__).parent.parent / "data" / "piacava"
OUT_FILE = DATA_DIR / "piacava_pevs.csv"
...
def main():
    url = build_url()
    print(f"Requisicao SIDRA (piacava):\n{url}\n")
    ...
    if n_prod > 0:
        print()
        print(df[df["quantidade_ton"].notna()].nlargest(5, "quantidade_ton")[
            ["municipio", "ano", "quantidade_ton", "valor_mil_reais"]
